...
function ToyClouds() {
  const clouds = [
    ...
    "left-[-4%] top-[32%] w-20 sm:w-32 [animation-delay:2.8s]",
...
function RsvpForm({ scriptUrl }: { scriptUrl: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await fetch(scriptUrl, {
        ...
        body: JSON.stringify({ nombre, asiste, acompanante, nombreAcompanante, mensaje }),
      });
      setStatus("success");
    } catch {
      setStatus("error");
  ...
  if (status === "success") {
    return (
      <div className="flex flex-col items-center gap-3 rounded-2xl border-2 border-ink bg-emerald/10 p-8 text-center">
        <span className="text-4xl">🎉</span>
        <p className="text-xl font-black text-emerald">¡Gracias {nombre}!</p>
        <p className="text-sm font-black uppercase tracking-wide text-cream">Tu respuesta fue registrada</p>
      </div>
    );
  }

  const toggleClass = (active: boolean) =>
    `flex-1 rounded-full border-2 border-ink py-2.5 text-sm font-black uppercase tracking-[0.1em] transition hover:-translate-y-0.5 ${
      active ? "bg-ink text-cream shadow-hard" : "bg-cream text-ink"
    }`;

  return (
    <form onSubmit={handleSubmit} className="grid gap-5">
      <div className="grid gap-2">
        <label className="text-xs font-black uppercase tracking-[0.18em] text-cream">Tu nombre</label>
        <input
          type="text"
          value={nombre}
          ...
          required
          className="w-full rounded-xl border-2 border-ink bg-white px-4 py-3 text-sm font-black placeholder:text-ink/30 focus:outline-none focus:ring-2 focus:ring-pink"
        />
      </div>

      <div className="grid gap-2">
        <label className="text-xs font-black uppercase tracking-[0.18em] text-cream">¿Confirmás asistencia?</label>
        <div className="flex gap-3">
          <button type="button" onClick={() => setAsiste("si")} className={toggleClass(asiste === "si")}>Sí, voy 🎉</button>
          <button type="button" onClick={() => setAsiste("no")} className={toggleClass(asiste === "no")}>No puedo 😢</button>
        </div>
      </div>
      ...
      {asiste === "si" && (
        <>
          <div className="grid gap-2">
            <label className="text-xs font-black uppercase tracking-[0.18em] text-cream">¿Venís con acompañante?</label>
            <div className="flex gap-3">
              <button type="button" onClick={() => setAcompanante("si")} className={toggleClass(acompanante === "si")}>Sí</button>
              <button type="button" onClick={() => setAcompanante("no")} className={toggleClass(acompanante === "no")}>No</button>
            </div>
          </div>

          {acompanante === "si" && (
            <div className="grid gap-2">
              <label className="text-xs font-black uppercase tracking-[0.18em] text-cream">Nombre del acompañante</label>
              <input
                type="text"
          ...
          <div className="grid gap-2">
            <label className="text-xs font-black uppercase tracking-[0.18em] text-cream">Preferencia de bebida / intolerancias</label>
            <textarea
              value={mensaje}
              onChange={(e) => setMensaje(e.target.value)}
              placeholder="Ej: sin gluten, solo vino blanco, etc."
              rows={3}
              className="w-full resize-none rounded-xl border-2 border-ink bg-white px-4 py-3 text-sm font-black placeholder:text-ink/30 focus:outline-none focus:ring-2 focus:ring-pink"
            />
          </div>
        </>
      )}
      ...
      <button
        type="submit"
        disabled={!nombre || !asiste || status === "sending"}
        className="w-full rounded-full border-2 border-ink bg-pink py-3.5 text-sm font-black uppercase tracking-[0.1em] text-cream shadow-hard transition hover:-translate-y-0.5 disabled:opacity-40 disabled:hover:translate-y-0"
      >
        {status === "sending" ? "Enviando..." : "Confirmar"}
      </button>

      {status === "error" && (
        <p className="text-center text-xs font-black text-red-500">Hubo un error. Intentá de nuevo.</p>
      )}
    </form>
  );
}
    ...
    <main className="relative min-h-screen overflow-x-hidden bg-emerald text-cream">
      ...
      <section className="hero-stage relative flex min-h-screen w-full items-center overflow-hidden px-4 py-10 sm:px-6">
        ...
        <div className="relative z-10 mx-auto flex w-full max-w-5xl flex-col items-center pt-28 text-center sm:pt-36 md:pt-44">
          ...
          <h1 className="birthday-title font-display animate-fade-up delay-2 max-w-5xl text-5xl font-black uppercase leading-[0.86] sm:text-7xl md:text-8xl lg:text-9xl" style={{color:"#ffe02f"}}>
            {INVITE.title}
            {/* top row */}
            <span className="sparkle" style={{"--dur":"1.8s","--delay":"-0.2s","--sz":"0.2em", top:"-0.35em", left:"2%"} as React.CSSProperties}>✦</span>
            <span className="sparkle" style={{"--dur":"2.2s","--delay":"-1.1s","--sz":"0.16em", top:"-0.4em", left:"14%"} as React.CSSProperties}>✧</span>
            <span className="sparkle" style={{"--dur":"1.9s","--delay":"-0.6s","--sz":"0.22em", top:"-0.38em", left:"28%"} as React.CSSProperties}>✦</span>
            <span className="sparkle" style={{"--dur":"2.4s","--delay":"-1.5s","--sz":"0.15em", top:"-0.3em", left:"43%"} as React.CSSProperties}>✧</span>
            <span className="sparkle" style={{"--dur":"2.0s","--delay":"-0.4s","--sz":"0.24em", top:"-0.42em", left:"58%"} as React.CSSProperties}>✦</span>
            <span className="sparkle" style={{"--dur":"1.7s","--delay":"-1.8s","--sz":"0.16em", top:"-0.32em", left:"73%"} as React.CSSProperties}>✧</span>
            <span className="sparkle" style={{"--dur":"2.1s","--delay":"-0.9s","--sz":"0.2em", top:"-0.38em", left:"88%"} as React.CSSProperties}>✦</span>
            {/* middle */}
            <span className="sparkle" style={{"--dur":"2.3s","--delay":"-0.3s","--sz":"0.18em", top:"40%", left:"-0.3em"} as React.CSSProperties}>✦</span>
            <span className="sparkle" style={{"--dur":"1.9s","--delay":"-1.3s","--sz":"0.14em", top:"55%", left:"20%"} as React.CSSProperties}>✧</span>
            <span className="sparkle" style={{"--dur":"2.0s","--delay":"-0.7s","--sz":"0.2em", top:"45%", left:"50%"} as React.CSSProperties}>✦</span>
            <span className="sparkle" style={{"--dur":"1.8s","--delay":"-1.6s","--sz":"0.15em", top:"50%", left:"78%"} as React.CSSProperties}>✧</span>
            <span className="sparkle" style={{"--dur":"2.2s","--delay":"-0.1s","--sz":"0.18em", top:"38%", right:"-0.3em"} as React.CSSProperties}>✦</span>
            {/* bottom row */}
            <span className="sparkle" style={{"--dur":"2.1s","--delay":"-0.5s","--sz":"0.16em", bottom:"-0.32em", left:"8%"} as React.CSSProperties}>✧</span>
            <span className="sparkle" style={{"--dur":"1.8s","--delay":"-1.0s","--sz":"0.22em", bottom:"-0.38em", left:"25%"} as React.CSSProperties}>✦</span>
            <span className="sparkle" style={{"--dur":"2.3s","--delay":"-1.7s","--sz":"0.15em", bottom:"-0.3em", left:"45%"} as React.CSSProperties}>✧</span>
            <span className="sparkle" style={{"--dur":"1.9s","--delay":"-0.8s","--sz":"0.2em", bottom:"-0.36em", left:"65%"} as React.CSSProperties}>✦</span>
            <span className="sparkle" style={{"--dur":"2.0s","--delay":"-1.4s","--sz":"0.17em", bottom:"-0.32em", left:"82%"} as React.CSSProperties}>✧</span>
          </h1>
      ...
      <section className="botanical-stage relative z-10 overflow-hidden px-4 py-14 sm:px-6 sm:py-16">
        ...
        <div className="relative z-10 mx-auto flex max-w-5xl flex-col gap-10">
          ...
          >
            <h2 className="script-message whitespace-normal text-base leading-tight sm:whitespace-nowrap sm:text-xl">
              que estos 36 vengan con suerte, llamemosla juntos 🍀
            </h2>
          </div>

          <div id="rsvp" className="grid gap-6 rounded-3xl bg-cream/92 p-5 text-ink shadow-hard backdrop-blur-md sm:p-7">
            <div>
            ...
            <RsvpForm scriptUrl={INVITE.scriptUrl} />
          </div>
